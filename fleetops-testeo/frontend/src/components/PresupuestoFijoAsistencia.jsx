...
const PresupuestoFijoAsistencia = ({ anio, buqueId, buqueNombre, onRefrescar }) => {
  ...
  useEffect(() => {
    ...
    const cargarDatos = async () => {
      ...
      const { data, error } = await supabase
        .from("presupuestos_fijos_asistencias") // <- tabla en plural
        .select("*")
        .eq("anio", anio)
        .eq("buque_id", buqueId);
  ...
  const guardar = async () => {
    ...
    // Buscar duplicados en todas las filas
  ...
  if (cargando) {
    return (
      <Flex align="center" justify="center" p={6}>
        <Spinner size="lg" />
        <Box ml={3}>Cargando presupuestos fijos de asistencias...</Box>
      </Flex>
    );
  }
    ...
    <Box mt={2}>
      <HStack mb={3} justify="space-between">
        <Heading size="sm" color="teal.700">
          🔧 Presupuestos Fijos Asistencias ({anio}) - {buqueNombre || buqueId}
        </Heading>
            ...
            <Tr key={fila.id ? fila.id : "nueva-" + idx}>
              <Td>
                <Input
                  ...
                  placeholder="Ej: Revisiones preventivas"
                  variant="flushed"
                  fontWeight="bold"
                />
              </Td>
      ...
      {registros.length === 0 && (
        <Box textAlign="center" color="gray.500" mt={8}>
          No hay presupuestos fijos de asistencias para este buque y año.<br />
...
export default PresupuestoFijoAsistencia;
